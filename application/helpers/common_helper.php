    ...
    function extractSearchParams($pagingParams, $type = 'default')
    {
        ...
        switch ($type)
        {
            ...
            case 'customer_listing':
                ...
                if (!empty($_GET['blacklistCustomer']))
                {
                    if($_GET['blacklistCustomer'] == "1")
                    {
                        $return['blacklistCustomer'] = 1;
                    }
                }
                break;
...
//        p($msg,0);
        ...
        $customer_name  = $record['customer_name'];
        $order_number  = $record['order_number'];
        $ship_onboard  = $record['ship_onboard'];
//        $to = '+6594561346';
        
        $select_to_replace = array("<CUSTOMER_NAME>", "<ORDER_NUMBER>", "<SOB_DATE>");
        $replace_with   = array("$customer_name", "$order_number", "$ship_onboard");

        $msg = str_replace($select_to_replace, $replace_with, $msg);
