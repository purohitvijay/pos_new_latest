...
//        p($this->db->last_query());
    ...
    public function checkOrderStatusReadyForJKT($shipment_batch_id)
    {
        ...
        $this->db->from('orders');
        $this->db->join('order_status_trans', "order_status_trans.order_id = orders.id and order_status_trans.active = 'yes'", 'left');
    ...
    public function updateOrderStatusReceivedAtJKT($shipment_batch_id)
    {
        ...
        if($query->num_rows() > 0)
        {
            ...
            foreach ($query->result_array() as $data)
            {
                ...
                $this->ordersModel->saveOrderStatus(array('order_id' => $order_id, 'active' => 'no', 'updated_by' => $this->session->userdata('id'), 'updated_at' => $time), array('order_id' => $order_id));
                
                $statuses = $this->config->item('jakarta_statuses');
                $responsibility_completed = $statuses['received_at_jakarta_warehouse']['responsibility_completed'];

                $update_order_status_data = array(
                    ...
                    'status' => 'received_at_jakarta_warehouse',
                    'updated_by' => $this->session->userdata('id'),
                    'order_id' => $order_id,
                );
                
                $order_status = $this->ordersModel->saveOrderStatus($update_order_status_data);
            }
        }
...
//            p($this->db->last_query());
    ...
    public function getAllShipmentBatches_r($pagingParams = array())
    {
        $this->db->select("shipment_batches.*, shipment_batches.batch_name as batch", FALSE);
        $this->db->from('shipment_batches');
//        $this->db->join('orders', 'shipment_batches.id = orders.shipment_batch_id', 'left');
//        $this->db->join('order_trans', 'orders.id = order_trans.order_id', 'left');
        $this->db->group_by('shipment_batches.id');
...
//            p($this->db->last_query());
    ...
    public function getShipmentBatches_box_order_count($shipmentBatchIds)
    {
