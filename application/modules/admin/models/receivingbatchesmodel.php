...
//            echo $this->db->last_query();exit;
    ...
    public function getAllReceivingBatches_r($pagingParams = array(), $type = 'open')
    {
        $this->db->select('receiving_batches.*,GROUP_CONCAT(DISTINCT batch_name SEPARATOR "<br/>") AS shipment_batches, GROUP_CONCAT(DISTINCT shipment_batches.`id` SEPARATOR ",") AS shipment_batches_id',false);
        $this->db->from('receiving_batches');
        $this->db->join('shipment_batches','receiving_batches.id = shipment_batches.receiving_batch_id','left');
//        $this->db->join('orders', 'shipment_batches.id = orders.shipment_batch_id', 'left');
//        $this->db->join('order_trans', 'orders.id = order_trans.order_id', 'left');
        ...
        if (!empty($pagingParams['iSortCol_0']))
        {
             $this->db->order_by('receiving_batches.id  desc');
...
//            echo $this->db->last_query();exit;
    ...
    public function getAllReceivingBatches_box_order_count($shipmentBatchIds)
    {
        $this->db->select('count(DISTINCT orders.id) as orders_count,
                sum(order_trans.quantity) as boxes_count',false);
        $this->db->from('shipment_batches');
        $this->db->join('orders', 'shipment_batches.id = orders.shipment_batch_id', 'left');
        $this->db->join('order_trans', 'orders.id = order_trans.order_id', 'left');
        $this->db->where("orders.`shipment_batch_id` IN($shipmentBatchIds)");
            $return = $this->db->get();
//            echo $this->db->last_query();exit;
            return $return->result_array();
    ...
    public function getActiveShipmentBatches($receiving_batch_id = null, $order_direction = null)
    {
        $result = array();
...
//            $this->db->where('status', 'no');
        ...
        {
            $this->db->order_by("id $order_direction");
        }
...
//            echo $this->db->last_query();exit;
    ...
    public function getShipmentOrdersPhoto($pagingParams = array(), $search_shipment_batch_id = null, $is_available = null, $search_receiving_batch_id = null, $is_include_luar_jawa = null)
    {
        
        $this->db->select("orders.id, orders.order_number, orders.jkt_received_date, order_image_trans.updated_at as date_uploaded, order_image_trans.status as image_status", FALSE);
        $this->db->from('orders');
...
//        $this->db->join("shipment_batches", "shipment_batches.id = orders.shipment_batch_id", "left");
        ...
        if (!empty($search_receiving_batch_id))
        {
            $this->db->where('shipment_batches.receiving_batch_id', $search_receiving_batch_id);
        }
        ...
        {
            $this->db->where('order_image_trans.status', null);
        }
        
        if (!empty($is_include_luar_jawa))
        {
            $this->db->where('order_trans.location_id !=', '6');
        }
        else
        {
//            $this->db->where('order_trans.location_id !=', '6');
...
//            $this->db->order_by('order_number','ASC');
            ...
            $this->db->order_by("(orders.jkt_received_date IS  NULL) , orders.`jkt_received_date` ASC" );
        }
