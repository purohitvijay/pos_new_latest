            ...
            foreach ($search as $search_param => $search_val)
            {
                switch ($search_param)
                {
                    ...
                    case 'residence_phone':
                        $this->db->like('residence_phone', $search_val);
                        break;

                    case 'pin':
                        $this->db->like('pin', $search_val);
                        break;
...
//        echo $this->db->last_query();exit;
    ...
    {
        $this->db->select("order_status_trans.*, user.name as employee, user.id as user_id, orders.jkt_received_date, jkt_reference_no, jkt_receiver", FALSE);
        $this->db->from('orders');
        $this->db->join("order_status_trans", "orders.id = order_status_trans.order_id");
        $this->db->join('user', 'user.id = order_status_trans.employee_id', 'left');
        $this->db->order_by('order_status_trans.id');
