...
class Receiving_batch extends MY_Controller
{
    ...
    public function getReceivingBatchesData()
    {
        ...
        $result = $this->receivingBatchesModel->getAllReceivingBatches($paginparam, 'all');
...
//        p($data);
    ...
    public function getShipmentBatchesJkt()
    {
        $this->load->model('receivingBatchesModel');
        $receiving_batch = $this->input->post('receiving_batch');
        $shipmentBatchesArr = $this->receivingBatchesModel->getActiveShipmentBatches($receiving_batch);
        $html = "";
        if(!empty($shipmentBatchesArr))
        {
