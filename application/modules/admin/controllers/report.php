...
//        p($dataArray['shipmentBatchesArr']);
    ...
    public function getPhotoReportsData()
    {
        ...
        $luarJawa = $_GET['luarJawa'];
        $search_receiving_batch_id = $_GET['search_receiving_batch_id'];
        $total = 0;
...
//        p($_GET);
        ...
        if (!empty($search_shipment_batch_id) || !empty($search_receiving_batch_id))
        {
            $result = $this->receivingBatchesModel->getShipmentOrdersPhoto($pagingParams, $search_shipment_batch_id, $is_available, $search_receiving_batch_id, $luarJawa);
//            p($result);
...
//                        $image_status = "Available";
                        $image_status = "<i class='fa fa-check-square'></i>";
                    }
...
//                    echo $date_uploaded;
                    ...
                    {
                       $date_uploaded = date("d/m/Y", strtotime($date_uploaded));
                    }
                    ...
                    $to_jkt_received_date_diffin_days = "N/A";
                    if($date_uploaded != "N/A" && $delivery_date != "N/A")
                    {
                        $to_jkt_received_date_diff = abs(strtotime($orderDetails['date_uploaded']) - strtotime($orderDetails['jkt_received_date']));
                        
                        $to_jkt_received_date_diffin_days = $to_jkt_received_date_diff/86400;  // 86400 seconds in one day

                        // and you might want to convert to integer
                        $to_jkt_received_date_diffin_days = intval($to_jkt_received_date_diffin_days);
                    }
                    //change if photo not taken but date delivered and jkt not blank
                    if($delivery_date != "N/A" && $date_uploaded == "N/A")
                    {
                        $to_jkt_received_date_diff = abs(strtotime(date('Y-m-d')) - strtotime($orderDetails['jkt_received_date']));
                    ...
                    $receivingBatchOrderData[$index]['delivery_date'] = $delivery_date;
                    $receivingBatchOrderData[$index]['date_uploaded'] = $date_uploaded;
                    $receivingBatchOrderData[$index]['image_status'] = $image_status;
                    $receivingBatchOrderData[$index]['date_uploaded_to_date_delivered'] = $to_jkt_received_date_diffin_days;
                }
                $days = array();
                foreach ($receivingBatchOrderData as $key => $row)
                {
                    $days[$key] = $row['date_uploaded_to_date_delivered'];
                }
                array_multisort($days, SORT_DESC, $receivingBatchOrderData);
            }
            else
            {
                $receivingBatchOrderData = array();
...
//        arsort($shipmentBatchesData);
        //change by reyansh
        $shipment_batches = $this->receivingBatchesModel->getReceivingBatchArr();
//        $dataArray = array();
        $shipmentBatchesData = array();
        if (!empty($shipment_batches))
        {
...
//        p($dataArray['records']);
    ...
    public function boxCollectionDateReport()
    {
        //load css
        $dataArray['local_css'] = array(
            'datatable', 'bootstrap_date_picker'
        ...
        $dataArray['records'] = $this->reportlib->getBoxCollectionDateReport($temp_date_from, $temp_date_to);
        $dataArray['collection_date_from'] = $collection_date_from;
        $dataArray['collection_date_to'] = $collection_date_to;
        $dataArray['temp_date_from'] = $temp_date_from;
        $dataArray['temp_date_to'] = $temp_date_to;


        $this->load->view('reports/boxCollectionDateReport', $dataArray);
    }
    
    public function downloadBoxCollectionDateReport($collection_date_from, $collection_date_to)
    {
        ini_set('memory_limit', '-1');
        $this->load->library('reportlib');
        $data = $this->reportlib->getBoxCollectionDateReport($collection_date_from, $collection_date_to);
        header("Content-Type: application/xls");
        header("Content-Disposition: attachment; filename=boxCollectionDateReport.xls");
        header("Pragma: no-cache");
        header("Expires: 0");
        echo "S.NO" . "\t";
        echo "Order Number" . "\t";
        ...
        foreach ($data as $idx => $value)
        {
            echo trim($idx + 1) . "\t";
            echo trim($value['order_number']) . "\t";
            echo trim($value['customer']) . "\t";
            echo trim($value['contacts']) . "\t";
            print("\n");
        }
    }
}
