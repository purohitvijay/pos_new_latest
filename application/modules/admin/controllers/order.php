...
//        p($data);
        $statutes = $this->config->item('consolidated_statuses');
        $check_blacklist = canPerformAction('blacklist', $this->_user_id);
        if ($data['foundRows'] > 0)
        {
            foreach ($data['resultSet'] as $idx => $val)
            {
                if (!empty($val['blacklistCustomer']))
                {
                    $data['resultSet'][$idx]['blacklist'] = "Yes";
                    $data['resultSet'][$idx]['blacklist_comment'] = $val['b_comment'];
                }
                else
                {
                    $data['resultSet'][$idx]['blacklist'] = "No";
                    $data['resultSet'][$idx]['blacklist_comment'] = "";
                }
    ...
    public function editCustomer($customer_id)
    {
        ...
        $userRec = $dataArray['data'];
        $searchBlacklist['mobile'] = $userRec['mobile'];
        $searchBlacklist['phone'] = $userRec['residence_phone'];

        $searchBlacklist['pin'] = $userRec['pin'];
        $blackListUser = $this->ordersmodel->checkCustomerBlacklistOrNot($searchBlacklist, "", 1);
//                        p($blackListUser,0);
        if (!empty($blackListUser))
        {
            $dataArray['blacklist'] = "Yes";
            $dataArray['blacklist_comment'] = $blackListUser['b_comment'];
            $dataArray['blacklist_customerId'] = $blackListUser['id'];
        }
        else
        {
            $dataArray['blacklist'] = "No";
            $dataArray['blacklist_comment'] = "";
            $dataArray['blacklist_customerId'] = 0;
        }
    ...
    {    
        ...
        $dataArray['check_blacklist'] = canPerformAction('blacklist', $this->_user_id);
...
//            if($check_blacklist == false)
//            {
                if(!empty($data['results']))
                {
                    foreach($data['results'] as $id => $userRec)
                    {
                        $data['results'][$id]['blacklist'] = "No";
...
//                        p($blackListUser,0);
                        if(!empty($blackListUser))
                        {
                            $data['results'][$id]['blacklist'] = "Yes";
                            $data['results'][$id]['blacklist_comment'] = $blackListUser['b_comment'];
                        }
                        else
                        {
                            $data['results'][$id]['blacklist'] = "No";
                            $data['results'][$id]['blacklist_comment'] = "";
                        }
                    }
                }
//            }
...
//            }
            ...
            $data['check_blacklist'] = $check_blacklist;
            $this->load->setTemplate('blank');
    ...
    public function saveCustomer($returnJson = false)
    {
        ...
        {
            if($this->input->post('blacklistCustomer'))
            {
                    $blacklistCustomer = $this->input->post('blacklistCustomer');
                    $bComment = $this->input->post('bcomments');
                    $array1 = array(
                        'mobile' => $mobile,
                        'residence_phone' => $phone,
                        'pin' => $pin,
                ...
                $blacklistCustomer = $this->ordersmodel->checkCustomerBlacklistOrNot($searchArr, "" , true);
                if(!empty($blacklistCustomer))
                {
                    $array1['id'] = $blacklistCustomer['id'];
                }
                    //save blacklist customer
                    $this->ordersmodel->saveBlacklistCustomer($array1);
            }
            else
            {  
            
            if($this->input->post('blacklistCustomerId'))
            {
                $blacklistCustomerId = $this->input->post('blacklistCustomerId');
                if($blacklistCustomerId > 0)
                {
                    $this->ordersmodel->deleteBlackListCustomer($blacklistCustomerId);
                }
            }
        }
        }
    ...
    public function getAddressByPinCodeFromDB()
    {
        ...
        $blacklistUserArr = $this->ordersmodel->checkCustomerBlacklistOrNot(array('pin' => $pincode));        
        $can_check_blacklist = canPerformAction('blacklist', $this->_user_id);
//        p($blacklistUserArr);
        $warning = "";
//        if($can_check_blacklist == false)
//        {
            if(!empty($blacklistUserArr))
            {
                foreach($blacklistUserArr as $idx => $customerRec)
                {
                    $warning .= $customerRec['mobile']."/".$customerRec['residence_phone']." or ".$customerRec['pin']." is under blacklisted customer ".$customerRec['b_comment']."</br>";
                }
            }
//        }
        $result->warning = $warning;
        $result->blacklist_user = $can_check_blacklist;
...
//                $configUpload['max_height'] = '1468'; #max height
    ...
    public function getBlacklistByPhoneFromDB()
    {
        $result = array();
        $this->load->model('ordersmodel');
        $phone = $this->input->post('phone');
        $mobile = $this->input->post('mobile');
        $searchBlacklist['mobile'] = $mobile;
        $searchBlacklist['phone'] = $phone;
        
        $can_check_blacklist = canPerformAction('blacklist', $this->_user_id);
        $warning = "";
//        if($can_check_blacklist == false)
//        {
            $blacklistUserArr = $this->ordersmodel->checkCustomerBlacklistOrNot($searchBlacklist, "OR");        
            if(!empty($blacklistUserArr))
            {
                foreach($blacklistUserArr as $idx => $customerRec)
                {
