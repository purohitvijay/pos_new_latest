...
class Masters extends MY_Controller
{
    ...
    public function getShipmentBatchData()
    {
        ...
        $result = $this->mastersModel->getAllShipmentBatches_r($paginparam);
        ...
        if (!empty($total))
        {
            foreach ($shipmentBatchData as $idx => $val)
            {
                ...
                //change by reyansh
                $orderCountVal = $this->mastersModel->getShipmentBatches_box_order_count($val['id']);
    ...
    //                    $shipmentBatchData[$idx]['delete'] = "--";
                    ...
                    if($canPerform === TRUE)
                    {
                        $shipmentBatchData[$idx]['operation']  .= "</br><a href='javaScript:void(0);' data_shipment_batch_id='". $val['id'] ."' class='bulkUpdateShipmentBatchStatus'><i class='fa fa-check-square'></i></a>";
                    }
                }
            }
        }
    ...
    public function bulkUpdateShipmentBatchStatus()
    {
        ...
        if($isOrderStatusReadyForJKT)
        {
            ...
            $re = $this->mastersModel->updateOrderStatusReceivedAtJKT($shipment_batch_id);
            
            if ($re) {
                
                $shipment_batch_row = $this->mastersModel->getShipmentBatchById($shipment_batch_id);

                // after update status need to close Shipment Batch status as well
                if (isset($shipment_batch_row->receiving_batch_id) && !empty($shipment_batch_row->receiving_batch_id)) {
                    $data = array(
                        'id' => $shipment_batch_row->receiving_batch_id,
                        'status' => 'closed',
                    );
                }
                $receiving_batch_id = $this->receivingBatchesModel->saveReceivingBatch($data);
            }
...
//        $this->form_validation->set_rules('load_date', 'Load Date', 'required|trim');j,/ >,
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            if (!empty($id))
            {
                ...
                $load_date = '';
                if(!empty($shipmentBatchRecord->load_date))
                {
...
//                'load_date' => $load_date,
            ...
            if($ship_onboard)
                $dataValues['ship_onboard'] = $ship_onboard;
            
            if($load_date)
                $dataValues['load_date'] = $load_date;
...
//            echo $ship_onboard_sms;
//            p($dataValues,0);
            if($ship_onboard_sms == 1)
            {
                $this->load->model('ordersModel');
                $orderArr = $this->ordersModel->getOrdersByShipmentBatchId($id);
                if(!empty($orderArr))
                {
                   foreach($orderArr as $idx => $orderRec)
                   {
                         $order = $this->ordersModel->getOrderDetails($orderRec['order_id']);
//                p($order,0);
                        $record = array();
                        $record['customer_name'] = $order['customer_name'];
                        $record['customer_number'] = $order['customer_mobile'];
                        $record['order_number'] = $order['order_number'];
                        if($dataValues['ship_onboard'] != "")
                        {
                            list($year, $month, $day) = explode('-', $dataValues['ship_onboard']);
                                $ship_onboard = "$day/$month/$year";
                        }
                        $record['ship_onboard'] = $ship_onboard;
//                        p($record);
...
//                            $quantity_diff = $new_quantity - $old_quantity;
   ...
   public function deleteMediaTypeById($id)
	...
	public function bulkUpdateShipmentBatchStatus_manual()
    {
	 $this->load->model('admin/mastersModel');	 
         $re = $this->mastersModel->updateOrderStatusReceivedAtJKT('1201');
         print_r($re);
    }
