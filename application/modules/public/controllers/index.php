...
//            $order_no = $this->session->userdata('order_number');
            ...
            $location = $OrderTransDetails[0]["location"];
...
//                        else
//                        {
                            $current_date = $order_data['resultSet'][$array_index_last]['created_at'];
//                            if($previousDate != "")
//                            {
...
//                        }
                    ...
                    if (isset($order_data['resultSet']))
                    {
                        ...
                        {                            
                            $previous_status[] = array("status" => $array_index[$i]['status'], "date" => $array_index[$i]['created_at']);
                        }
                        if($current_status == "collected_at_warehouse" && $ship_on_board != "")
                        {
                            $previous_status[] = array("status" => "ready_for_receiving_at_jakarta", "date" => $array_index[$i]['created_at']);
                        }
                        if($current_status == "collected_at_warehouse" && $ship_on_board == "")
                        {
                            $shipped = 1;
...
//  *** If In Array ($order_data['resultSet']) Get jkt_reference_no. Add Static Array In (previous_filter_status)                
                ...
                //}
                $jkt_received_date = $order_data['resultSet'][$array_index_last]['jkt_received_date'];
            ...
            if(!empty($current_filter_status))
            {
                ...
                foreach($current_filter_status as $idx => $statusRec)
                {
                    ...
                    if($statusRec['status'] == 'delivered_at_jkt_picture_not_taken' )
                    {
                        ...
                        {
                            $current_filter_status[$idx]['next_status'] = "Jakarta Image Link";
...
//            p($current_filter_status);
            $order_result = array(
                ...
                'receiver_date' => isset($jkt_received_date) ? date("d/m/Y", strtotime($jkt_received_date)) : false,
                'reference_no' => isset($reference_no) ? $reference_no : false,
...
//  *** If jkt_received_date Field IN Order Table Is Not Equal TO (0000-00-00) Then Create A Static Status "recipient_received" ***           
                if ($received_date != '0000-00-00' && $received_date != null)
                {
                    ...
                    if (isset($order_data['resultSet']))
                    {
                        ...
                        {
                            $previous_status[] = array("status" => $rec['status'], "date" => $rec['created_at']);
                        }
                    }
                }
                else
